...
def get_login_info():
    ...
    if user == None or passw == None:
        ...
        if os.path.exists(credentials_file):
            with open(credentials_file, "r") as f:
                ...
                if len(lines) >= 2:
                    ...
                    if username.lower() == "username" and password.lower() == "password":
                        print("Default credentials detected. Using YFinance instead")
                        #this return is added to ditch wrds and switch to yfin if no credentials.
                        return "error", "error"
                        username = input("Enter WRDS username: ")
                        password = getpass("Enter WRDS password: ")
                else:
                    print("Invalid credentials file format. Using YFinance instead")
                    #this return is added to ditch wrds and switch to yfin if no credentials.
                    return "error", "error"
                    username = input("Enter WRDS username: ")
                    password = getpass("Enter WRDS password: ")
            ...
            print("Credentials file not found. Using YFinance instead")
            #this return is added to ditch wrds and switch to yfin if no credentials.
            return "error", "error"
            username = input("Enter WRDS username: ")
            password = getpass("Enter WRDS password: ")
